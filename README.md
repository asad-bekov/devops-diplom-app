...
HTML-контент приложения вынесен в ConfigMap и монтируется в nginx-контейнер в виде volume.

Обновление пользовательского интерфейса выполняется без пересборки Docker-образа — путём обновления ConfigMap и перезапуска Deployment.
...
- **Kubernetes-кластер**:
  - Self-hosted, развёрнут с помощью `kubeadm`
  - Минимум 3 ноды (1 control-plane + 2 worker)
  - Версия: **1.28+**
  - Операционная система нод: **Ubuntu 22.04 LTS**
- Установленные локально инструменты:
  - `kubectl`
  - `docker`
  - `git`
...
4. **Ingress**:
   ...
   - **Назначение**: маршрутизация HTTP-трафика к приложению
   - **Динамический DNS**: используется сервис [`nip.io`](https://nip.io), который автоматически преобразует IP-адрес в доменное имя без необходимости настройки DNS.
     - Пример host: `devops-diplom-app.178.154.224.117/.nip.io`
   - **Внешний IP-адрес** ingress-контроллера можно получить командой:

```bash
kubectl get svc ingress-nginx-controller -n ingress-nginx
```
   - После получения EXTERNAL-IP, полное доменное имя формируется по шаблону: `<APP_NAME>.<EXTERNAL_IP>.nip.io`
...
Доступность приложения проверена с помощью `curl`, что подтверждает корректную работу сервиса.
...
*Данные меры и рекомендации приведены в рамках учебного дипломного проекта и не претендуют на полноту production-решения.*
...
Общий обзор Kubernetes-кластера и ресурсов, используемых приложением в namespace `cicd`.
...
Состояние pod'ов приложения `devops-diplom-app` в namespace `cicd`.
...
![Application UI](screenshots/02-application/app-ui.png)
---
...
Успешный запуск CI/CD pipeline GitHub Actions, выполняющего сборку и деплой приложения.
